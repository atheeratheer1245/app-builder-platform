...
السبب هو أن Google Cloud لا يحتوي بعد على رابط العودة الذي يرسله الموقع المنشور ضمن **عميل OAuth النشط**.

معرّف العميل النشط الذي تحقّق الخادم من سره هو:

```text
271495009963-n86689drhqhmkqgkoc221ifs3e335a39.apps.googleusercontent.com
```
...
حتى آخر اختبار، ما زال Google يعيد `redirect_uri_mismatch`. يعني ذلك أن الرابط لم يُحفظ في عميل OAuth النشط أعلاه، أو أن المستخدم لم يصل بعد إلى إعدادات العميل. يجب إضافته إلى **العميل المرتبط بالـ Client ID المذكور في هذه الوثيقة**، لا إلى عميل OAuth آخر أو عميل محذوف.

تمت مراجعة ترويسة التحويل الصادرة من الموقع مباشرةً، وهي ترسل هذا الرابط حرفيًا ضمن `redirect_uri`:
