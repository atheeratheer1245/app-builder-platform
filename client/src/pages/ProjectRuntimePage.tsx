...
function ProjectScreen({ components, pages, activePageId, onNavigate, isArabic }: { components: RuntimeComponent[]; pages: RuntimePage[]; activePageId: number; onNavigate: (pageId: number) => void; isArabic: boolean }) {
  ...
  return <div className="relative overflow-hidden rounded-3xl p-1" style={{ backgroundColor }}>{backgroundType === "image" && backgroundUrl ? <><div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${backgroundUrl})` }} /><div className="absolute inset-0 bg-white/80" /></> : null}{backgroundType === "video" && backgroundUrl ? <><video className="absolute inset-0 h-full w-full object-cover" autoPlay muted loop playsInline src={backgroundUrl} /><div className="absolute inset-0 bg-white/80" /></> : null}{backgroundType === "audio" && backgroundUrl ? <audio className="sr-only" autoPlay loop src={backgroundUrl} /> : null}<div className="relative z-10 space-y-4">{currentComponents.map(component => {
    const props = asRecord(component.properties);
